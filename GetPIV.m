function [VX,VY] = GetPIV(im1,im2,X1,Y1,EdgeLength)
    ...
    %   Output are components of the flow field on the grid
    %   
    ...
    for i = 1 : nb(1)
        for j = 1 : nb(2)
            ...
            counter = counter+1;
            temp1 = im1(max(1,((i-1)*EdgeLength-window+1)):min((i+1)*EdgeLength+window,si(1)),(max(1,(j-1)*EdgeLength-window+1)):min((j+1)*EdgeLength+window,si(2)));
            temp2 = im2(max(1,((i-1)*EdgeLength-window+1)):min((i+1)*EdgeLength+window,si(1)),(max(1,(j-1)*EdgeLength-window+1)):min((j+1)*EdgeLength+window,si(2)));    
...
%             temp2 = im2(max(1,((i-1)*EdgeLength-window+1)):min((i+1)*EdgeLength+window,si(1)),(max(1,(j-1)*EdgeLength-window+1)):min((j+1)*EdgeLength+window,si(2)),:);    
%             
%             shift = xcorr3fft(temp1,temp2);
%             shiftx(counter) = shift(1);
%             shifty(counter) = shift(2);
%             
